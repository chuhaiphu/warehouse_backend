    ...
    public ExportRequestResponse confirmCountedExportRequest(String exportRequestId) {
        ...
        if (exportRequest.getType().equals(ExportType.INTERNAL)) {
            ...
            if (hasLackStatus) {
                ...
                renewExportRequest(new RenewExportRequestRequest(
                        exportRequest.getId(),
                        exportRequest.getExportRequestDetails().stream()
                                .filter(detail -> detail.getStatus() != DetailStatus.LACK)
                                .map(detail -> new RenewExportRequestRequest.itemList(
                                        detail.getItem().getId(),
                                        detail.getActualQuantity(),
                                        detail.getActualMeasurementValue()
                                ))
                                .toList()
                ));
            } else {
